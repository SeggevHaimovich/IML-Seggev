...
def _deal_with_invalid_data(df: pd.DataFrame):
    mask = df[
        (df["Day"] <= 0) | (df["Day"] > 31) | (df["Month"] <= 0) |
        (df["Month"] > 12) | ((df["Day"] > 29) & (df["Month"] == 2)) |
        ((df["Day"] > 30) & ((df["Month"] == 4) | (df["Month"] == 6) |
...
if __name__ == '__main__':
    ...
    fig = px.scatter(il_data, x="DayOfYear", y="Temp", color="StrYear",
                     title="Temperature as function of Day of The Year")
    fig.write_image(os.path.join(IMG_PATH, "Temp-DayOfYear" + ".png"),
                    format="png", engine="orca")

    months = np.arange(1, 13)
    per_month = np.array(il_data.groupby("Month").Temp.agg('std'))
    fig = go.Figure(data=go.Bar(x=months, y=per_month, 
                                text=np.round(per_month, 2)),
                    layout=go.Layout(title="Temperature STD for each Month",
                                     xaxis_title="Month", yaxis_title="STD"))
    fig.write_image(os.path.join(IMG_PATH, "Temp(std)-Month" + ".png"),
                    format="png", engine="orca")

    # Question 3 - Exploring differences between countries

    groups = df.groupby(["Country", "Month"]).Temp.agg(['std', 'mean']).\
        reset_index()
    fig = px.line(groups, x="Month", y='mean', color="Country", error_y="std",
                  title="Average and STD temperature for each Month")
    fig.write_image(os.path.join(IMG_PATH, "AVG-Month" + ".png"), format="png",
                    engine="orca")
    ...
    train_X, train_y, test_X, test_y = \
        split_train_test(il_data.DayOfYear, il_data.Temp,
    ...
    fig.write_image(os.path.join(IMG_PATH, "Loss-Degree(Israel)" + ".png"),
                    format="png", engine="orca")

    # # Question 5 - Evaluating fitted model on different countries

    not_il_data = df.drop(df[df.Country == "Israel"].index).\
    ...
    fig = go.Figure(data=go.Bar(x=countries, y=losses,
                                text=np.round(losses, 2)),
                    layout=go.Layout(
                        title=f"Loss as function of "
                              f"country with k = {CHOSEN_K}",
                        xaxis_title="Country", yaxis_title="Loss"))
    fig.write_image(os.path.join(IMG_PATH, "Loss-Country" + ".png"),
                    format="png", engine="orca")
